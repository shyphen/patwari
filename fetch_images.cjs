...
async function fetchImages() {
    try {
        ...
        const response = await axios.get(url, {
            headers: { 'User-Agent': 'HungriDhabaBot/1.0 (contact@example.com)' }
        });
        ...
        for (const pageId in pages) {
            ...
            if (page.thumbnail && page.thumbnail.source) {
                ...
                const imgRes = await axios.get(imgUrl, { responseType: 'stream', headers: { 'User-Agent': 'HungriDhabaBot/1.0 (contact@example.com)' } });
